...
from sdk.secret import SecretSource, SecretValue
...
# 目前agent启动之后，就会加载一次，但正确的应该是每次基于用户消息进行加载
# 且不应该重复加载，所以需要在agent context中记录已经加载的skill名称
class AgentContext(BaseModel):
    ...
    # 这样会报错，因为pydantic无法解析其schema
    memory_manager: MemoryManager | None = Field(
        default=None, description="记忆管理 - 支持记忆的加载"
    )
    ...
    def get_user_memory(
        ...
        need_load_memory: bool,
        activate_ids: list[str] | None = None,
    ) -> tuple[TextContent, list[str]] | None:
        """基于用户消息，依次加载用户的基本信息、经验信息等记忆内容，并进行格式化"""
        # 从本地文件中读取
        base_memory: str
        # 调用es服务，通过关键词+向量检索获取top5 数据
        ...
        formatted_memory_text = render_template(
            ...
            need_load_memory=need_load_memory,
