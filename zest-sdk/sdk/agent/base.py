...
from sdk.context.condenser import CondenserBase
...
from sdk.mcp import create_mcp_tools
...
from sdk.utils.models import DiscriminatedUnionMixin


if TYPE_CHECKING:
    from sdk.agent.runner_context import RunnerContext
    from sdk.conversation import ConversationState
...
class AgentBase(DiscriminatedUnionMixin, ABC):
    ...
    custom_system_prompt: str | None = Field(
        default=None,
        description=(
            "Custom system prompt string. If provided, this will be used directly "
            "instead of loading from system_prompt_filename. This allows SubAgents "
            "to have flexible custom prompts without needing template files."
        ),
    )
    ...
    def system_message(self) -> str:
        ...
        # If custom_system_prompt is provided, use it directly
        if self.custom_system_prompt:
            system_message = self.custom_system_prompt
            # Still append agent_context suffix if available
        ...
        # Otherwise, use the template-based approach
    ...
    def _initialize(self, state: ConversationState):
        ...
        if self.filter_tools_regex:
            ...
            tools = [tool for tool in tools if pattern.match(tool.name)]  # type: ignore
            logger.info(
        ...
        self._tools = {tool.name: tool for tool in tools}  # type: ignore
    ...
    def verify(
        ...
        for tool_class_name in self.include_default_tools:
            ...
            if tool_class is not None:
                runtime_names.add(tool_class.tool_name)

        for tool_class_name in persisted.include_default_tools:
            tool_class = BUILT_IN_TOOL_CLASSES.get(tool_class_name)
            if tool_class is not None:
                persisted_names.add(tool_class.tool_name)
