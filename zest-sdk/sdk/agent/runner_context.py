...
class RunnerContext(BaseModel):
    ...
    # ===== 会话级共享数据（直接字段）=====
    id: ConversationID = Field(..., description="会话唯一标识")
    ...
    def blocked_actions(self, value: dict[str, str]) -> None:
        """设置当前 Agent 的被阻止动作"""
        self.agent_state.blocked_actions = value
    ...
    def blocked_messages(self, value: dict[str, str]) -> None:
        """设置当前 Agent 的被阻止消息"""
        self.agent_state.blocked_messages = value
    ...
    def publish_event(self, event: Any) -> None:
        ...
        self.event_center.publish(
            event=event, conversation_id=str(self.id), agent_id=str(self.agent_id)
        )
    # ===== 构建方法 =====
    @classmethod
    def build(
        ...
        # 设置私有属性
        context._fs = conversation_state._fs
        context._cipher = conversation_state._cipher
