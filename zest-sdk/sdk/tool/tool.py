...
from sdk.security import risk
from sdk.tool.schema import Action, Observation, Schema
from sdk.utils.models import (
...
# 【修改1】删除TYPE_CHECKING下的LocalConversation导入，无需再引用
if TYPE_CHECKING:
    from sdk.agent.runner_context import (
        RunnerContext,
    )  # 保留原有RunnerContext导入
...
class ToolAnnotations(BaseModel):
    ...
    model_config: ClassVar[ConfigDict] = ConfigDict(
        frozen=True,
        title="sdk.tool.tool.ToolAnnotations",
    )
...
class ToolExecutor[ActionT, ObservationT](ABC):
    ...
    def __call__(
        ...
        Args:
            ...
            context: The runner context for the tool execution.  # 【修改3】注释：conversation→context，移除LocalConversation相关说明
        """
...
class ExecutableTool(Protocol):
    ...
    def __call__(
        ...
        action: Action,
        context: RunnerContext
        | None = None,  # 【修改4】参数名+类型：conversation→context，LocalConversation→RunnerContext
    ) -> Observation:
...
class ToolDefinition[ActionT, ObservationT](DiscriminatedUnionMixin, ABC):
    ...
    model_config: ClassVar[ConfigDict] = ConfigDict(
        frozen=True, arbitrary_types_allowed=True, validate_assignment=False
    )
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if "tool_name" not in cls.__dict__:
            cls.tool_name = _camel_to_snake(cls.__name__).removesuffix("_tool")
    ...
    def model_post_init(self, __context: Any) -> None:
        """Pydantic原生后置钩子：实例化完成后自动执行（核心逻辑）
        如果定义了类属性，那么强制使用类属性覆盖实例化的属性
        - 优化-如果未定义name，则考虑类tool_name，如果tool_name也未定义，则根据类名自动生成name（驼峰转蛇形，移除_tool后缀）
        """
        if self.name and not self.name.strip():
            return

        # 没有设置name时，自动根据类名生成驼峰命名的工具名（移除_tool后缀）
        if not self.name or not self.name.strip():  # 空值判断（None/空字符串/纯空格）
            # 填充驼峰命名，移除_tool后缀
            # 冻结模型下安全赋值：用Pydantic原生方法，避免直接赋值报错
            object.__setattr__(self, "name", self.tool_name)
    ...
    def __call__(
        self, action: ActionT, context: RunnerContext | None = None
    ) -> Observation:
        ...
        # 【修改5】核心修复：executor调用传参从conversation→context，和ToolExecutor的参数名保持一致
        result = self.executor(action, context)
