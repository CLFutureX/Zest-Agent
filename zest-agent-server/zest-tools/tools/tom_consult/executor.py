...
from common.storage.event_log import EventLog, create_event_log
...
class TomConsultExecutor(
    ...
    def __call__(
        ...
        if isinstance(action, SleeptimeComputeAction):
            return self._sleeptime_compute(context)
        else:
            return self._consult_tom(action, context)
    ...
    def _format_events(
        ...
        if context is not None:
            ...
            return context.llm.format_messages_for_llm(messages)[1:]
        else:
            # If no conversation, format messages directly from events
            from sdk.llm import TextContent
    ...
    def _consult_tom(
        self, action: ConsultTomAction, context: RunnerContext | None = None
    ) -> ConsultTomObservation:
        ...
        try:
            ...
            formatted_messages = []
            if context is not None:
                formatted_messages = self._format_events(context.events, context)
    ...
    def _sleeptime_compute(
        self, context: RunnerContext | None = None
    ) -> SleeptimeComputeObservation:
        ...
        similar to the Zest implementation.
        ...
        for session_id in sessions_to_process:
            session_data = self._extract_session_data(session_id, context)
            if session_data:
    ...
    def _extract_session_data(
        self, session_id: str, context: RunnerContext | None = None
    ) -> dict[str, Any] | None:
        """Extract session data from a conversation directory."""

        # Load events from the session using file_store
        #events_dir = f"{self.conversations_dir}/{session_id}/events"
        main_agent = True if context.agent_state.parent_agent_id is None else False
        events = create_event_log(fs=self.file_store,  conversation_id=context.conversation_id,agent_id = context.agent_id
                                  ,main_agent=main_agent)

        # Format events into messages
        formatted_messages = self._format_events(events, context)
