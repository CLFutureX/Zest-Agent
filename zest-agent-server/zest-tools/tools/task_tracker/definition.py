...
class TaskTrackerExecutor(ToolExecutor[TaskTrackerAction, TaskTrackerObservation]):
    ...
    Task list is stored in context.agent_state.todos (list[dict]) so it
    persists with the AgentState checkpoint instead of a separate file.
    """
    ...
    def __call__(
        ...
        if action.command == "plan":
            # Persist directly into agent_state - no file IO needed
            # 注意的点： llm会判断任务完成之后，重新给于完整的todos，
            context.agent_state.todos = [t.model_dump() for t in action.task_list]
            return TaskTrackerObservation.from_text(
                text=(
                    f"Task list has been updated with {len(action.task_list)} item(s)."
                ),
                command=action.command,
                task_list=action.task_list,
            )
        elif action.command == "view":
            # Read from agent_state
            task_list = [
                TaskItem.model_validate(t) for t in context.agent_state.todos
            ]
            if not task_list:
                return TaskTrackerObservation.from_text(
            ...
            content = self._format_task_list(task_list)
            return TaskTrackerObservation.from_text(
                text=content,
                command=action.command,
                task_list=task_list,
    ...
    def _format_task_list(self, task_list: list[TaskItem]) -> str:
        ...
        for i, task in enumerate(task_list, 1):
            ...
            out += f"{i}. {status_icon} {task.title}\n"
            if task.notes:
                out += f"   {task.notes}\n"
            out += "\n"
...
class TaskTrackerTool(ToolDefinition[TaskTrackerAction, TaskTrackerObservation]):
    ...
    @classmethod
    def create(cls, context: RunnerContext) -> Sequence["TaskTrackerTool"]:
        """Initialize TaskTrackerTool with a TaskTrackerExecutor.

        Args:
            context: Runner context (used for tool lifecycle; task state
                     is stored in context.agent_state.todos).
        """
