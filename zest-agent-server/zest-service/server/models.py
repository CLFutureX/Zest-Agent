...
from server.utils import ZestAgentUUID, utc_now
from sdk import LLM, AgentBase, Event, ImageContent, Message, TextContent
...
from common.utils.models import DiscriminatedUnionMixin, ZestAgent
...
class SendMessageRequest(BaseModel):
    ...
    This is a simplified version of sdk.Message.
...
class StartConversationRequest(BaseModel):
    ...
    """Internal request model used by zest-service runtime."""
    ...
    confirmation_policy: ConfirmationPolicyBase = Field(
        ...
        description="Controls when the conversation will prompt the user before continuing.",

    )

    security_analyzer: SecurityAnalyzerBase  = Field(

        default= LLMSecurityAnalyzer(),

        description="Optional security analyzer used to evaluate actions during the conversation.",
    ...
    tool_module_qualnames: dict[str, str] = Field(
        ...
        description=(
            ...
            "Mapping of tool names to their module qualnames from the client's registry."
    ...
    plugins: list[PluginSource] | None = Field(
        ...
        description=(
            ...
            "List of plugins to load for this conversation. Plugins are loaded and merged into the agent."
    ...
    hook_config: HookConfig | None = Field(
        ...
        description="Optional hook configuration for this conversation.",

    )


class ConversationAccessInfo(BaseModel):
    conversation_id: str
    base_url: str
    events_url: str
    websocket_url: str
...
class BashEventBase(DiscriminatedUnionMixin, ABC):
    ...
    id: ZestAgentUUID = Field(default_factory=uuid4)
