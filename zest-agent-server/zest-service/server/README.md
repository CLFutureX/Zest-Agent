...
uv run python -m server --host localhost --port 3000
...
| `ZEST_SECRET_KEY` | Secret key for encrypting sensitive data (LLM API keys, secrets) in stored conversations. **Required for persistence across restarts.** | None |
...
The server encrypts sensitive data (such as LLM API keys and conversation secrets) when storing conversations to disk. To enable this encryption and ensure secrets persist across server restarts, you **must** set the `ZEST_SECRET_KEY` environment variable.

#### Setting ZEST_SECRET_KEY

```bash
# Generate a secure random key (recommended)
export ZEST_SECRET_KEY=$(openssl rand -hex 32)

# Or set a custom key
export ZEST_SECRET_KEY="your-secret-key-here"
```
...
- Without `ZEST_SECRET_KEY`, secrets will be redacted (not encrypted) and will be lost on restart

#### What Gets Encrypted

The following fields are encrypted when `ZEST_SECRET_KEY` is set:
...
#### Behavior Without ZEST_SECRET_KEY

If `ZEST_SECRET_KEY` is not set:
- The server will log a warning: `⚠️ ZEST_SECRET_KEY was not defined. Secrets will not be persisted between restarts.`
...
# Run with coverage
uv run pytest tests/agent_server/ --cov=server
```

## Security Considerations

- **Authentication**: Use `session_api_key` in production environments
- **Secret Encryption**: Always set `ZEST_SECRET_KEY` in production to encrypt sensitive data
...
5. **LLM API keys are None after restart**: This happens when `ZEST_SECRET_KEY` is not set or has changed. Set `ZEST_SECRET_KEY` before starting the server to encrypt and persist secrets. Note: If you change the key, previously encrypted secrets cannot be decrypted.
