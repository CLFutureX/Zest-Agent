...
# from server.extends.bash_router import get_default_bash_event_service
from server.config import get_default_config
from server.conversation_service import get_default_conversation_service
from server.models import ExecuteBashRequest, Success
...
async def download_trajectory(
    ...
    """Download a file from the workspace."""
    config = get_default_config()
    temp_file = config.conversations_path / f"{conversation_id}.zip"
    conversation_dir = config.conversations_path / conversation_id
