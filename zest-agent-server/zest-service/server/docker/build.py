...
from common.logger import IN_CI, get_logger, rolling_log_view
from sdk.workspace import PlatformType, TargetType
...
def _default_sdk_project_root() -> Path:
    """
    Resolve top-level Zest UV workspace root:
    ...
    def _is_workspace_root(d: Path) -> bool:
        ...
        _EXPECTED = (
            "Zest-sdk/pyproject.toml",
            "Zest-tools/pyproject.toml",
            "Zest-workspace/pyproject.toml",
            "Zest-agent-server/pyproject.toml",
        )
    ...
    def validate(p: Path, src: str) -> Path:
        ...
        if not _is_workspace_root(root):
            raise RuntimeError(
                f"{src}: couldn't find the Zest UV workspace root "
                f"starting at '{p}'.\n\n"
    ...
    raise RuntimeError(
        "Could not resolve the Zest UV workspace root.\n\n"
        "Expected repo layout:\n"
        "  pyproject.toml  (with [tool.uv.workspace].members "
        "including Zest/* subprojects)\n"
        "  Zest-sdk/pyproject.toml\n"
        "  Zest-tools/pyproject.toml\n"
        "  Zest-workspace/pyproject.toml\n"
        "  Zest-agent-server/pyproject.toml\n\n"
...
def _package_version() -> str:
    """
    Get the semantic version from the Zest-sdk package.
    This is used for versioned tags during releases.
    """
    try:
        from importlib.metadata import version

        return version("Zest-sdk")
    except Exception:
        # If package is not installed, try reading from pyproject.toml
        try:
            sdk_root = _default_sdk_project_root()
            pyproject_path = sdk_root / "Zest-sdk" / "pyproject.toml"
...
class BuildOptions(BaseModel):
    ...
    image: str = Field(default="ghcr.io/Zest/agent-server")
    ...
    sdk_project_root: Path = Field(
        ...
        description="Path to Zest SDK root. Auto if None.",
    )
...
def _default_local_cache_dir() -> Path:
    ...
    return Path(xdg) / "Zest" / "buildx-cache"
    ...
    if push:
        ...
        if driver == "docker-container":
            ...
            logger.warning(
                ...
                "  1. docker buildx create --name Zest-builder "
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--image",
        default=_env("IMAGE", "ghcr.io/Zest/agent-server"),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to Zest SDK root (default: auto-detect).",
    )
    parser.add_argument(
        "--build-ctx-only",
        action="store_true",
