...
Resolver = Callable[[dict[str, Any], "RunnerContext"], Sequence[ToolDefinition]]
"""A resolver produces ToolDefinition instances for given params.
...
def _resolver_from_instance(name: str, tool: ToolDefinition) -> Resolver:
    ...
    def _resolve(
        params: dict[str, Any], context: "RunnerContext"
...
def _resolver_from_callable(
    ...
    def _resolve(
        ...
        try:
            ...
            created = factory(context=context, **params)
...
def _resolver_from_subclass(_name: str, cls: type[ToolDefinition]) -> Resolver:
    ...
    def _resolve(
        ...
        created = create(context=context, **params)
        if not isinstance(created, Sequence) or not all(
            isinstance(t, ToolDefinition) for t in created
        ):
            raise TypeError(
...
def resolve_tool(
    tool_spec: Tool, context: "RunnerContext"
) -> Sequence[ToolDefinition]:
    with _LOCK:
    ...
    return resolver(tool_spec.params, context)
