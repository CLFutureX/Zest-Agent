...
class HookConfig(BaseModel):
    ...
    Hooks can be configured either by loading from `.zest_hook/hooks.json` or
    by directly instantiating with typed fields:
        ...
        # Load from JSON file:
        config = HookConfig.load(".zest_hook/hooks.json")
    """
    ...
    def load(
        ...
        """Load config from path or search .zest_hook/hooks.json locations.

        Args:
            path: Explicit path to hooks.json file. If provided, working_dir is ignored.
            working_dir: Project directory for discovering .zest_hook/hooks.json.
        ...
        if path is None:
            ...
            search_paths = [
                base_dir / ".zest_hook" / "hooks.json",
                Path.home() / ".zest_hook" / "hooks.json",
            ]
