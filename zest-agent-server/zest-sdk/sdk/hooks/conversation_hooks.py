...
from sdk.event import ActionEvent, Event, MessageEvent, ObservationEvent
from sdk.hooks.config import HookConfig
from sdk.hooks.manager import HookManager
from sdk.hooks.types import HookEventType
...
## todo 暂时只针对mainAgent的行为做拦截。后期可进一步扩展，到时候只需要改造onevent，携带agent_id即可获取要目标agent_state
class HookEventProcessor:
    ...
    def _handle_pre_tool_use(self, event: ActionEvent) -> None:
        ...
        if not should_continue:
            ...
            if self._conversation_state is not None:
                ...
                self._conversation_state.main_agent_state.block_action(event.id, block_reason)
            else:
                logger.warning(
    ...
    def _handle_post_tool_use(self, event: ObservationEvent) -> None:
        ...
        if self._conversation_state is not None:
            try:
                idx = self._conversation_state.main_agent_state.events.get_index(event.action_id)
    ...
    def _handle_user_prompt_submit(self, event: MessageEvent) -> MessageEvent:
        ...
        if not should_continue:
            ...
            if self._conversation_state is not None:
                block_reason = reason or "Blocked by hook"
                self._conversation_state.main_agent_state.block_message(event.id, block_reason)
            else:
                logger.warning(
    ...
    def is_action_blocked(self, action_id: str) -> bool:
        ...
        if self._conversation_state is None:
            return False
        return action_id in self._conversation_state.main_agent_state.blocked_actions

    def is_message_blocked(self, message_id: str) -> bool:
        """Check if a message was blocked by a hook."""
        if self._conversation_state is None:
            return False
        return message_id in self._conversation_state.main_agent_state.blocked_messages
