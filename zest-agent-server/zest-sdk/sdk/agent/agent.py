...
from common.observability.utils import extract_action_name
...
class Agent(AgentBase):
    """Main agent implementation for Zest.
    ...
    def init_state(
        ...
        logger.debug(
            f"init_state called: conversation_id={context.conversation_id}, "
        ...
        if has_system_prompt:
            ...
            logger.warning(
                ...
                f"conversation_id={context.conversation_id}, event_count={len(events)}. "
        ...
        if has_user_message:
            ...
            logger.error(
                ...
                f"conversation_id={context.conversation_id}, events={event_types}"
            )
    ...
    def step(
        ...
        for event in reversed(list(context.events)):
            if isinstance(event, MessageEvent) and event.source == "user":
                ...
                if reason is not None: 
                    ...
                    context.agent_state.set_execution_status(ExecutionStatus.FINISHED)
                    return
        ...
        logger.info(
            ...
            f"{json.dumps([m.model_dump() for m in _messages], indent=2,ensure_ascii=False)}"
        )
    ...
    def _get_action_event(
        ...
        tool: ToolDefinition | None = self.tools_map.get(tool_name, None)
    ...
    def _execute_action_event(
        ...
        try:
            if should_enable_observability():
                ...
                observation: Observation = observe(name=tool_name, session_id=context.conversation_id, 
                                                   user_id = context.user_id, span_type="TOOL")(
                    tool
    ...
    def _log_context_window_exceeded_warning(self) -> None:
        ...
        if self.condenser is None:
            logger.warning(
                ...
                "https://docs.Zest.dev/sdk/guides/context-condenser\n"
                "=" * 80
            )
        else:
