...
from sdk.git.cached_repo import try_cached_clone_or_update
from common.logger import get_logger


if TYPE_CHECKING:
    from sdk.context.skills.skill import Skill, SkillResources
...
def load_and_categorize(
    ...
    # AgentSkills (SKILL.md directories) are a separate category from Zest skills.
...
def get_skills_cache_dir() -> Path:
    ...
    Returns:
        Path to the skills cache directory (~/.Zest/cache/skills).
    """
    cache_dir = Path.home() / ".Zest" / "cache" / "skills"
...
def update_skills_repository(
    ...
    Uses the shared git caching infrastructure from sdk.git.cached_repo.
...
def discover_skill_resources(skill_dir: Path) -> SkillResources:
    ...
    """
    # Import here to avoid circular dependency
    from sdk.context.skills.skill import SkillResources
