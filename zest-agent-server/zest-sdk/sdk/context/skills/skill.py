...
from sdk.context.skills.exceptions import SkillError, SkillValidationError
from sdk.context.skills.trigger import (
    KeywordTrigger,
    TaskTrigger,
)
from sdk.context.skills.types import InputMetadata
...
class Skill(BaseModel):
    ...
    This model supports both Zest-specific fields and AgentSkills standard
    ...
    def load(
        ...
        Supports both Zest-specific frontmatter fields and AgentSkills
        ...
        if path.name.lower() == "skill.md":
            ...
            return cls._load_legacy_Zest_skill(path, file_content, skill_base_dir)
    ...
    @classmethod
    def _load_legacy_Zest_skill(
        cls, path: Path, file_content: str, skill_base_dir: Path | None
    ) -> "Skill":
        """Load a skill from a legacy Zest-format file.
...
def load_skills_from_dir(
    ...
    - Zest format: skills/*.md files
    - AgentSkills format: skills/skill-name/SKILL.md directories

    Note, legacy repo instructions will not be loaded here.

    Args:
        skill_dir: Path to the skills directory (e.g. .Zest/skills)
...
USER_SKILLS_DIRS = [
    Path.home() / ".Zest" / "skills",
    Path.home() / ".Zest" / "microagents",  # Legacy support
]
...
def load_user_skills() -> list[Skill]:
    ...
    Searches for skills in ~/.Zest/skills/ and ~/.Zest/microagents/
...
def load_project_skills(work_dir: str | Path) -> list[Skill]:
    ...
    Searches for skills in {work_dir}/.Zest/skills/ and
    {work_dir}/.Zest/microagents/ (legacy). Skills from both
    ...
    # This ensures they are loaded even if .Zest/skills doesn't exist
    ...
    # Load project-specific skills from .Zest/skills and legacy microagents
    project_skills_dirs = [
        work_dir / ".Zest" / "skills",
        work_dir / ".Zest" / "microagents",  # Legacy support
    ]
...
PUBLIC_SKILLS_REPO = "https://github.com/Zest/skills"
...
def load_public_skills(
    ...
    """Load skills from the public Zest skills repository.

    This function maintains a local git clone of the public skills registry at
    https://github.com/Zest/skills. On first run, it clones the repository
    to ~/.Zest/skills-cache/. On subsequent runs, it pulls the latest changes
    ...
    Example:
        >>> from sdk.context import AgentContext
        >>> from sdk.context.skills import load_public_skills
        >>>
        >>> # Load public skills
