...
from sdk.context.condenser.utils import (
...
from sdk.event.base import LLMConvertibleEvent
...
class LLMSummarizingCondenser(RollingCondenser):
    ...
    """
    _total_token_cache: int = PrivateAttr(0)  # 累计总token数
    _last_event_id: str | None = PrivateAttr(None)  # 最后一次统计的事件ID
    _last_event_idx: int | None = PrivateAttr(None)  # 最后一次统计的事件索引
    ...
    def get_condensation_reasons(
        ...
        # 每次都统计，其实没必要，可以缓存，实现增量计算
        if self.max_tokens and agent_llm:
            if self._last_event_id and self._last_event_idx is not None:
                # Count tokens for new events since the last count
                new_events = view.events[self._last_event_idx + 1 :]
                new_tokens = get_total_token_count(new_events, agent_llm)
                self._total_token_cache += new_tokens
            else:
                # If the last counted event is no longer in the view, we need to
                # recalculate the total token count for all events.
                self._total_token_cache = get_total_token_count(view.events, agent_llm)
            self._last_event_id = view.events[-1].id if view.events else None
            self._last_event_idx = len(view.events) - 1 if view.events else None
            # 记录最近的events，在压缩时，对集合进行清空即可。
