...
from sdk.llm.utils.model_info import get_litellm_model_info
from common.utils.deprecation import warn_deprecated
...
from sdk.llm.mixins.non_native_fc import NonNativeToolCallingMixin
from sdk.llm.options.chat_options import select_chat_options
from sdk.llm.options.responses_options import select_responses_options
from sdk.llm.utils.metrics import Metrics, MetricsSnapshot
from sdk.llm.utils.model_features import get_default_temperature, get_features
from sdk.llm.utils.retry_mixin import RetryMixin
from sdk.llm.utils.telemetry import Telemetry
from common.logger import ENV_LOG_DIR, get_logger
...
class LLM(BaseModel, RetryMixin, NonNativeToolCallingMixin):
    """Language model interface for Zest agents.
    ...
    embedding_model: str = Field(
        default="dashscope/text-embedding-v1", description="embedding model name."
    )
    ...
    openrouter_app_name: str = Field(default="Zest")
    ...
    model_canonical_name: str | None = Field(
        ...
        description=(
            ...
            "The Zest SDK maintains a model feature registry that "
    ...
    def _coerce_inputs(cls, data):
        ...
        # Provider rewrite: Zest/* -> litellm_proxy/*
        if model_val.startswith("Zest/"):
            model_name = model_val.removeprefix("Zest/")
    ...
    def completion(
        ...
        **kwargs,
        ...
        Example:
            >>> from sdk.llm import Message, TextContent
        ...
        try:
            ...
            # Convert the first choice to an Zest Message
    ...
    def responses(
        ...
        **kwargs,
        ...
        if kwargs.get("stream", False) or self.stream or event_center is not None:
        ...
        **kwargs,
        ...
        with self._litellm_modify_params_ctx(self.modify_params):
            with warnings.catch_warnings():
                ...
                if enable_streaming and event_center is not None:
                    assert isinstance(ret, CustomStreamWrapper)
                    chunks = []
                    for chunk in ret:
                        event_center.publish(chunk)
                        chunks.append(chunk)
    ...
    def get_embedding(self, texts: list[str]) -> list[list[float]]:
        try:
            ...
            logger.error(
                f"Error get_embedding for model {self.embedding_model}\n {texts} \n error:{e}",
                exc_info=True,
            )
            raise Exception("embedding 异常")
