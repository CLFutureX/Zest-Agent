...
class AgentConfigService:
    ...
    async def _resolve_llm(
        ...
        if llm_config_id:
            ...
            llm_resource = await self.llm_config_storage.get_llm_config(llm_config_id)
    ...
    async def _resolve_skills(self, user_id: str, skill_ids: list[str]) -> list[SkillDefinitionPayload]:
        ...
        if skill_ids:
            ...
            for skill_id in skill_ids:
                ...
                skill = await self.skill_storage.get_skill(skill_id)
            ...
            source_skills = await self.skill_storage.list_skills_by_user(user_id, enabled=True)
    ...
    async def _resolve_prompts(self, user_id: str, prompt_ids: list[str]) -> list[str]:
        ...
        if prompt_ids:
            ...
            for prompt_id in prompt_ids:
                ...
                prompt = await self.prompt_storage.get_prompt(prompt_id)
            ...
            source_prompts = await self.prompt_storage.list_prompts_by_user(user_id, enabled=True)
    ...
    async def _resolve_subagents(
        ...
        source_configs = await self.subagent_config_storage.list_subagent_configs_by_user(
